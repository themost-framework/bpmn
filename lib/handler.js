    ...
    if (handler) {
        ...
        if (handlerType === 'function') {
            try {
                ...
                var handlerResult = process.defaultErrorHandler.call(process.processClient, error, done);
            }
            ...
            result = process.callDefaultEventHandler(eventType, name, mapName2HandlerName(name),
                                            "Unknown handler type: '" + handlerType + "'", done);
        }
     } else {
        result = process.callDefaultEventHandler(eventType, name, mapName2HandlerName(name), "No handler found", done);
    }
    // support promise
